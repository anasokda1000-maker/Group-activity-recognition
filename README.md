...
## Overview
In this project we got a data consisting of volleyball olympic clips where there are 41 sequential frames in each clip. The frame is a photo of the court and the players and some other Classification-irrelevant factors like crowd. each clip is classified into one of eight group activity classes and each player in each frame is classified into one nine person action classes. Our purpose is to make a group activity recognition model that classify each clip into one of eight classes utilizing both the temporal dynamic of frames in the clip and the person actions in the clip.

## Background
There are two different types of classification the group classification and the individual classification. The individual classification depends on the spatial features and temporal dynamics of the individual itself whilst the group activity classification depends on each person action classification in the group and the relation between them and not utilizing that information makes the model more naive with lower performance and this is where the idea of hierarchical models came from which was proposed first by "Hierarchical Deep Temporal Models for Group Activity Recognition" (Ibrahim et al., 2016). The hierarchical model classifies the group activity based on each individual in the group and the relation between them to end up with a single classification on the group activity. 
